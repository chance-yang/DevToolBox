...
// SubtleCrypto does not support MD5; this provides parity with other hashes.
// Algorithm: RFC 1321. Adapted from blueimp-md5 (MIT) for Unicode-safe input.
...
function bytesToWords(bytes: Uint8Array): number[] {
  ...
  for (let i = 0; i < bytes.length; i += 4) {
    words[i >> 2] =
      bytes[i] | ((bytes[i + 1] ?? 0) << 8) | ((bytes[i + 2] ?? 0) << 16) | ((bytes[i + 3] ?? 0) << 24);
  }
  return words;
}
...
function md5blk(bytes: Uint8Array): number[] {
  ...
  const totalWords = Math.ceil(bytes.length / 4);
...
function md5(bytes: Uint8Array): string {
  ...
  // Pad to 64-byte boundary leaving room for 8-byte length
